...
// (4) 绑定基本路径到服务容器 instances 属性中
    ...
    {
        // 通过 $this->config['app.providers'] 取 app.php 中 providers 数组（放的是 Laravel 全部服务提供者）
        // 根据 Illuminate 字符串切割为系统服务类和用户服务类
        $providers = Collection::make($this->config['app.providers'])
                        ->partition(function ($provider) {
                            return Str::startsWith($provider, 'Illuminate\\');
                        });
        // 把 PackageManifest 类中的服务提供者也加入到集合中
...
// (43) 记录服务提供者已经加载过
    ...
    public function boot()
    {
        ...
        // 核心
        array_walk($this->serviceProviders, function ($p) { // array_walk 用户自定义函数对数组中的每个元素做回调处理
            // 看这些服务提供者对象有没有 boot 方法，如果有，则执行一下这个方法，如果没有，就跳过，继续处理下一个服务提供者
            $this->bootProvider($p);
        });
